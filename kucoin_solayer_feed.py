...
def taapi_candles(backtracks: int = SNAP_LEN) -> pd.DataFrame:
    ...
    while fetched < backtracks:
        want = min(CHUNK, backtracks - fetched)
                params = {
            "secret":   SECRET,
            "exchange": EXCHANGE,       # e.g. binance or binancefutures
            "symbol":   SYMBOL,         # LAYER/USDT  (spot)  |  LAYERUSDT (perp)
            "interval": INTERVAL,       # 15m
            "backtrack": fetched,       # offset from most recent candle
            "range":     want,          # how many candles to return (≤99 on free tier)
            "format":    "JSON",
        }

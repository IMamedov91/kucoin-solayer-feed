...
• _identiek_ JSON‑formaat aan de BTC‑feed  ⇢  last_300_candles toont alléén OHLCV
"""
...
def fetch_frame(symbol: str, tf: str) -> pd.DataFrame:
    ...
    while len(raw) < FETCH_LEN:
        params = (
            {"symbol": symbol, "type": tf,
             "limit": MAX_LIMIT, "endAt": end_ms}                 # spot
            if ENDPOINT == "spot"
            else {"symbol": symbol, "granularity": int(tf),
                  "limit": MAX_LIMIT, "to": end_ms}               # futures
        )
        batch = _get(params)
        if not batch:
            break
...
_clean = lambda v: None if (isinstance(v,float) and (math.isnan(v)|math.isinf(v))) else v

# alleen de velden die we óók in de BTC‑feed tonen

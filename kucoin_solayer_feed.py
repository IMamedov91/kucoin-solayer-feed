#!/usr/bin/env python3
"""
kucoin_solayer_feed.py — v6‑fix3
• Spot óf futures feed voor SOLayer via env ENDPOINT
• Correcte paginatie‑parameter:  endAt (spot)  |  to (futures)
• Granularity in minuten (futures: 15)
• Kolom‑mapping aangepast: futures = 6 velden, spot = 7
...
TF_DEFAULT     = "15min" if ENDPOINT == "spot" else "15"   # KuCoin notatie
SYMBOL_DEFAULT = "SOLAYER-USDT" if ENDPOINT == "spot" else "SOLAYERUSDTM"

if ENDPOINT == "futures":                                 # max 500 per call
    API_URL, MAX_LIMIT = (
        "https://api-futures.kucoin.com/api/v1/kline/query", 500)
else:                                                      # max 150 per call
    API_URL, MAX_LIMIT = (
        "https://api.kucoin.com/api/v1/market/candles", 150)

FETCH_LEN    = 550          # 300 snapshot + buffer zodat indicatoren compleet zijn
SNAPSHOT_LEN = 300
...
def _get(params: dict, retries: int = 3) -> list[list[t.Any]]:
    """GET‑wrapper met eenvoudige retry‑logica."""
...
def fetch_frame(symbol: str, tf: str) -> pd.DataFrame:
    ...
    while len(raw) < FETCH_LEN:
        ...
        if not batch:
            break
        raw.extend(batch)
        end_ms = int(batch[-1][0]) - MS_PER_BAR  # naar vorige candle‑slot

    # kolom‑lijst afhankelijk van endpoint
    cols = (
        ["ts", "open", "close", "high", "low", "vol", "turnover"] if ENDPOINT == "spot" else
...
# ─────────────────────── utilities ────────────────────────────────
iso    = lambda ms: dt.datetime.fromtimestamp(ms / 1000, dt.timezone.utc).isoformat(timespec="seconds") + "Z"
...
def push_gist(token: str, gist_id: str, fname: str, payload: dict) -> None:
    """Uploadt JSON‑payload naar een Gist‑bestand."""
    requests.patch(
